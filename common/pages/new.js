...
async function renderBody(html, page = "1") {
  const requestUrl = `https://api.hackerwebapp.com/newest?newest=${page}`;
  try {
    const { data, isOffline } = await apiFetch(requestUrl);
    return newsList(html, { news: data, page, heading: "Newest", isOffline });
  } catch (e) {
...
export default async function renderPage({ html }, params, { page = "1" }) {
  ...
  return layout(html, { body, title: "Newest" });
}
